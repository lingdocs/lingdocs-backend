...
apiRouter.put("/user/tests", async (req, res, next) => {
  if (!req.user) return next("user not found");
  try {
    const { tests } = req.body as AT.PostTestResultsBody;
...
apiRouter.put("/user/upgrade", async (req, res, next) => {
  ...
  try {
    ...
    if (givenPassword.toLowerCase().trim() !== studentPassword.toLowerCase()) {
      const wrongPass: AT.UpgradeUserResponse = {
        ok: false,
    ...
    if (user.level !== "basic") {
      const alreadyUpgraded: AT.UpgradeUserResponse = {
        ok: true,
    ...
    const upgraded: AT.UpgradeUserResponse = await upgradeUser(userId);
    res.send(upgraded);
  } catch (e) {
    next(e);
  }
