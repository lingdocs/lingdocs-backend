...
export const outsideProviders: ("github" | "google" | "twitter")[] = [
  "github",
  "google",
  "twitter",
];
...
function setupPassport(passport: PassportStatic) {
    ...
    new LocalStrategy(
      ...
      async function (username, password, done) {
        try {
          ...
          if (!user.password)
            return done(null, false, { message: "user doesn't have password" });
          compare(password, user.password, (err, result) => {
            if (err) return done(err);
            if (result === true) {
              const u = updateLastLogin(user);
              insertLingdocsUser(u)
                .then((usr) => {
  ...
  passport.use(
    new GoogleStrategy(
      {
        clientID:
          "1059009861653-ndh517ctpnats30qlgmihsrol4pdcj12.apps.googleusercontent.com",
        ...
        try {
          if (req.isAuthenticated()) {
            ...
            if (!u.email) {
              ...
              if (email) {
                const emailAdded = await updateLingdocsUser(req.user.userId, {
                  email,
                  emailVerified: true,
                });
          ...
          if (googleMail.email) {
            const otherAccountWSameEmail = await getLingdocsUser(
              "email",
              googleMail.email,
            );
            if (otherAccountWSameEmail) {
              await updateLingdocsUser(otherAccountWSameEmail.userId, {
                google: gProfile,
              });
    ...
    new TwitterStrategy(
      ...
      async function (req, token, tokenSecret, profileRaw, done) {
        ...
        try {
          if (req.isAuthenticated()) {
            if (!req.user) done(new Error("user lost"));
            const otherAccountWSameTwitter = await getLingdocsUser(
              "twitterId",
              twitterProfile.id,
            );
  ...
  passport.use(
    new GitHubStrategy(
      ...
      async function (
        ...
        const { _json, _raw, ...profile } = profileRaw;
        const ghProfile: AT.GitHubProfile = { ...profile, accessToken };
        try {
          if (req.isAuthenticated()) {
            if (!req.user) done(new Error("user lost"));
            const otherAccountWSameGithub = await getLingdocsUser(
              "githubId",
              ghProfile.id,
            );
