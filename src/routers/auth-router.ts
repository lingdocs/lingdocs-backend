...
const authRouter = (passport: PassportStatic) => {
  ...
  router.post(
    ...
    async (req, res, next) => {
      try {
        ...
        const userId = req.params.userId as AT.UUID;
  ...
  router.delete("/admin/:userId", async (req, res, next) => {
    try {
      ...
      const toDelete = req.params.userId as AT.UUID;
...
function getTestCompletionSummary(users: AT.LingdocsUser[]) {
...
function removeDuplicateTests(tests: AT.TestResult[]): AT.TestResult[] {
