...
export function getVerifiedEmail({
  emails,
}: AT.ProviderProfile): string | false {
  return emails &&
    emails.length &&
    // @ts-ignore
    emails[0].verified
...
export function getEmailFromGoogleProfile(profile: AT.GoogleProfile): {
...
): Promise<AT.DowngradeUserResponse> {
...
export async function denyUserUpgradeRequest(userId: AT.UUID): Promise<void> {
...
export async function createNewUser(
  input:
    ...
    | {
        strategy: "github";
        profile: AT.GitHubProfile;
      }
    | {
        strategy: "google";
        profile: AT.GoogleProfile;
      }
    | {
        strategy: "twitter";
        profile: AT.TwitterProfile;
      },
